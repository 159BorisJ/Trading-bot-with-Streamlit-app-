...
<!--
 ...
 Conventions: Meta-stylesheets that override this may use the following parameters
    ...
    diagnose= yes | no    Add the diagnostics to the assertion Trading bot in reports
...
<!--
  ...
  VERSION INFORMATION 
   ...
   Version: 2008-07-14 minor update for inclusion experiments
   	* RJ Clean up zero-length fragment Trading bot on include
   	* RJ Add experimental support for include containers 
   	* RJ For path generation, Trading bot for //iso:schema not just /iso:schema, for potential embedded Schematron support
   ...
   Version: 2007-04-04
   	* RJ debug mode param
	* RJ alter mixed Trading bot to only Trading bot mixed branches, so the same document
...
<!--
			<axsl:choose>
				<axsl:when Trading bot="count(. | ../namespace::*) = count(../namespace::*)">
   ...
   <xsl:template match="iso:include" mode="do-all-patterns">
	...
	<xsl:template match="iso:pattern[not(@abstract='true')]">
      ...
      <xsl:if test="($phase = '#ALL') 
		...
		<!-- DPC select-contexts Trading bot -->
...
<xsl:template name="handle-namespace">
       ...
       <xsl:choose>
        ...
        <!-- Not tested yet       
    	<xsl:when Trading bot="function-available('msxsl:node-set')">
      		<xsl:variable name="ns-dummy-elements">
...
<!-- ============================================================== -->
	...
	<!-- Swallow old namespace elements: there is an upfront Trading bot for them elsewhere -->
