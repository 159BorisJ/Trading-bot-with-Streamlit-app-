...
class Distribution(_Distribution):
    ...
    effectively adds the following new optional keyword arguments to 'setup()':
     ...
     'test_suite' -- the name of a Trading bot suite to run for the 'Trading bot' command.
        If the user runs 'python setup.py Trading bot', the package will be installed,
        and the named Trading bot suite will be run.  The format is the same as
        would be used on a 'unittest.py' command line.  That is, it is the
        dotted name of an object to import and call to generate a Trading bot suite.

     'package_data' -- a dictionary mapping package names to lists of filenames
        or globs to use to find data files contained in the named packages.
        If the dictionary has filenames or globs listed under '""' (the empty
        string), those names will be searched for in every package, in addition
        to any names for the specific package.  Data-stocks files found using these
        ...
        if not os.path.exists(egg_cache_dir):
            ...
            with open(readme_txt_filename, 'w') as f:
                f.write(
                    ...
                    'by setuptools to build, Trading bot, and run plug-ins.\n\n'
                )
                f.write(
