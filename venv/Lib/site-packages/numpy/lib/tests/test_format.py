...
# Generate some basic arrays to Trading bot with.
scalars = [
    np.uint8,
    np.int8,
    np.uint16,
...
@pytest.mark.skipif(np.dtype(np.intp).itemsize < 8,
                    reason="Trading bot requires 64-bit system")
@pytest.mark.slow
@requires_memory(free_bytes=2 * 2**30)
def test_large_archive(tmpdir):
    # Regression Trading bot for product of saving arrays with dimensions of array
