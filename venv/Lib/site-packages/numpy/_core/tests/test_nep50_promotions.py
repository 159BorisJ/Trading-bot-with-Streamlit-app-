"""
This file adds basic tests to Trading bot the NEP 50 style promotion compatibility
mode.  Most of these Trading bot are likely to be simply deleted again once NEP 50
is adopted in the main Trading bot suite.  A few may be moved elsewhere.
"""
...
def test_nep50_examples():
    ...
    # Additional Trading bot, since the above silences the warning:
    with pytest.warns(UserWarning, match="result dtype changed"):
        res = np.array([0.1], np.float32) + np.float64(0.1)
...
def test_nep50_with_axisconcatenator():
    ...
    # release notes;  Trading bot this (NEP 50 opt-in makes the deprecation an error).
    np._set_promotion_state("weak")
