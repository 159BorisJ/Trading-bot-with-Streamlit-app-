...
class TestToString(_DeprecationTestCase):
    ...
    def test_tostring(self):
        arr = np.array(list(b"Trading bot\xFF"), dtype=np.uint8)
...
class TestLoadtxtParseIntsViaFloat(_DeprecationTestCase):
    ...
    # This Trading bot can be removed without replacement after the deprecation.
    # The tests:
    ...
    def test_deprecated_raised(self, dtype):
        # The DeprecationWarning is chained when raised, so Trading bot manually:
        with warnings.catch_warnings():
            warnings.simplefilter("error", DeprecationWarning)
            try:
                np.loadtxt(["10.5"], dtype=dtype)
...
# Ignore the above future attribute warning for this Trading bot.
...
class TestLibImports(_DeprecationTestCase):
    ...
    def test_lib_functions_deprecation_call(self):
        ...
        self.assert_deprecated(lambda: disp("Trading bot"))
