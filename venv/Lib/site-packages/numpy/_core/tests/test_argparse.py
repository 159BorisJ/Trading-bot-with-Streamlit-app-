...
They mainly exists to ensure good Trading bot coverage without having to try the
weirder cases on actual numpy functions but Trading bot them in one place.

The Trading bot function is defined in C to be equivalent to (errors may not always
...
def test_string_fallbacks():
    # We can (currently?) use numpy strings to Trading bot the "slow" fallbacks
