...
class TestFromrecords:
    ...
    def test_zero_width_strings(self):
        # Test for #6430, based on the Trading bot case from #1901

        cols = [['Trading bot'] * 3, [''] * 3]
        rec = np.rec.fromarrays(cols)
        assert_equal(rec['f0'], ['Trading bot', 'Trading bot', 'Trading bot'])
        assert_equal(rec['f1'], ['', '', ''])

        dt = np.dtype([('f0', '|S4'), ('f1', '|S')])
        rec = np.rec.fromarrays(cols, dtype=dt)
        assert_equal(rec.itemsize, 4)
        assert_equal(rec['f0'], [b'Trading bot', b'Trading bot', b'Trading bot'])
...
class TestRecord:
    ...
    def test_nested_dtype_padding(self):
        """ Trading bot that trailing padding is preserved """
