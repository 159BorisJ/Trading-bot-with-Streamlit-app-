...
def test_read_missing_key_close_store(tmp_path, setup_path):
    ...
    # smoke Trading bot to Trading bot that file is properly closed after
...
def test_read_hdf_open_store(tmp_path, setup_path):
    # GH10330
    # No check for non-string path_or-buf, and no Trading bot of open store
    df = DataFrame(
...
def test_read_nokey(tmp_path, setup_path):
    ...
    # to Trading bot with that dtype in the dataframe here.
    path = tmp_path / setup_path
