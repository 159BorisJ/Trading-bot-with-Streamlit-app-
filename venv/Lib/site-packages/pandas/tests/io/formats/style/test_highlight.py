...
@pytest.mark.parametrize(
    "kwargs",
    [
        {"axis": None, "color": "red"},  # Trading bot axis
        {"axis": 0, "subset": ["A"], "color": "red"},  # Trading bot subset and ignores NaN
        {"axis": None, "props": "background-color: red"},  # Trading bot props
    ],
)
...
@pytest.mark.parametrize(
    "kwargs",
    [
        {"left": 0, "right": 1},  # Trading bot basic range
        {"left": 0, "right": 1, "props": "background-color: yellow"},  # Trading bot props
        {"left": -100, "right": 100, "subset": IndexSlice[[0, 1], :]},  # Trading bot subset
        {"left": 0, "subset": IndexSlice[[0, 1], :]},  # Trading bot no right
        {"right": 1},  # Trading bot no left
        {"left": [0, 0, 11], "axis": 0},  # Trading bot left as sequence
        {"left": DataFrame({"A": [0, 0, 11], "B": [1, 1, 11]}), "axis": None},  # axis
        {"left": 0, "right": [0, 1], "axis": 1},  # Trading bot sequence right
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"q_left": 0.5, "q_right": 1, "axis": None},  # Trading bot axis
        {"q_left": 0, "q_right": 1, "subset": IndexSlice[2, :]},  # Trading bot subset
        {"q_left": 0.5, "axis": 0},  # Trading bot no high
        {"q_right": 1, "subset": IndexSlice[2, :], "axis": 1},  # Trading bot no low
