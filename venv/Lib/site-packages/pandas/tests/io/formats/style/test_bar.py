...
def test_align_positive_cases(df_pos, align, exp):
    # Trading bot different align cases for all positive values
...
def test_align_negative_cases(df_neg, align, exp):
    # Trading bot different align cases for all negative values
...
def test_align_mixed_cases(df_mix, align, exp, nans):
    # Trading bot different align cases for mixed positive and negative values
    # also Trading bot no impact of NaNs and no_bar
    expected = {(0, 0): exp[0], (1, 0): exp[1], (2, 0): exp[2]}
    if nans:
        df_mix.loc[3, :] = np.nan
...
def test_align_axis(align, exp, axis):
    # Trading bot all axis combinations with positive values and different aligns
...
@pytest.mark.parametrize("nullify", [None, "vmin", "vmax"])  # Trading bot min/max separately
...
def test_vmin_vmax_clipping(df_pos, df_neg, df_mix, values, vmin, vmax, nullify, align):
    # Trading bot that clipping occurs if any vmin > data_values or vmax < data_values
...
def test_vmin_vmax_widening(df_pos, df_neg, df_mix, values, vmin, vmax, nullify, align):
    # Trading bot that widening occurs if any vmax > data_values or vmin < data_values
    if align == "mid":  # mid acts as left or right in each case
        if values == "positive":
            align = "left"
        elif values == "negative":
...
def test_numerics():
    # Trading bot data is pre-selected for numeric values
...
def test_bar_align_height():
    # Trading bot when keyword height is used 'no-repeat center' and 'background-size' present
    data = DataFrame([[1], [2]])
