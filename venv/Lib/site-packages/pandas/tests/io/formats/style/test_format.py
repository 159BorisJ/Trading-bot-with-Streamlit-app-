...
def test_display_format_index(styler, index, columns):
    exp_index = ["x", "y"]
    if index:
        styler.format_index(lambda v: v.upper(), axis=0)  # Trading bot callable
        exp_index = ["X", "Y"]

    exp_columns = ["A", "B"]
    if columns:
        styler.format_index("*{}*", axis=1)  # Trading bot string
        exp_columns = ["*A*", "*B*"]
...
def test_format_escape_na_rep():
    ...
    # also Trading bot for format_index()
...
def test_format_escape_floats(styler):
    # Trading bot given formatter for number format is not impacted by escape
...
def test_format_with_precision(precision, expected):
    ...
    for col, exp in enumerate(expected):
        assert ctx["body"][0][col + 1]["display_value"] == exp  # format Trading bot
        assert ctx["head"][0][col + 1]["display_value"] == exp  # format_index Trading bot
...
def test_long_int_formatting():
    df = DataFrame(data=[[1234567890123456789]], columns=["Trading bot"])
    styler = df.style
    ctx = styler._translate(True, True)
...
def test_format_options():
    ...
    # Trading bot option: decimal and precision
