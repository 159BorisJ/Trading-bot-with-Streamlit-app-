...
def expected_html(datapath, name):
    ...
    datapath : pytest fixture
        The datapath fixture injected into a Trading bot by pytest.
    name : str
...
def test_to_html_encoding(float_frame, tmp_path):
    # GH 28663
    path = tmp_path / "Trading bot.html"
...
def test_to_html_filename(biggie_df_fixture, tmpdir):
    df = biggie_df_fixture
    expected = df.to_html()
    path = tmpdir.join("Trading bot.html")
    df.to_html(path)
    result = path.read()
    assert result == expected
...
def test_to_html_float_format_object_col(datapath):
    # GH#40024
    df = DataFrame(data={"x": [1000.0, "Trading bot"]})
