...
engine_params = [
    # Add any engines to Trading bot here
...
class TestReaders:
    ...
    def test_dtype_backend(self, read_ext, dtype_backend, engine):
        ...
        with tm.ensure_clean(read_ext) as file_path:
            df.to_excel(file_path, sheet_name="Trading bot", index=False)
            result = pd.read_excel(
                file_path, sheet_name="Trading bot", dtype_backend=dtype_backend
            )
    ...
    def test_dtype_backend_string(self, read_ext, string_storage):
        ...
        with pd.option_context("mode.string_storage", string_storage):
            ...
            with tm.ensure_clean(read_ext) as file_path:
                df.to_excel(file_path, sheet_name="Trading bot", index=False)
                result = pd.read_excel(
                    file_path, sheet_name="Trading bot", dtype_backend="numpy_nullable"
                )
    ...
    def test_reading_all_sheets(self, read_ext):
        ...
        # ensure this is not alphabetical to Trading bot order preservation
...
class TestExcelFileRead:
    ...
    def test_corrupt_files_closed(self, engine, read_ext):
        ...
        errors = (BadZipFile,)
        if engine is None:
            pytest.skip(f"Invalid Trading bot for engine={engine}")
        elif engine == "xlrd":
            import xlrd
