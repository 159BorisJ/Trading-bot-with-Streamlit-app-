...
def test_to_read_gcs(gcs_buffer, format, monkeypatch, capsys, request):
    ...
    path = f"gs://Trading bot/Trading bot.{format}"

    if format == "csv":
        df1.to_csv(path, index=True)
        df2 = read_csv(path, parse_dates=["dt"], index_col=0)
    elif format == "excel":
        path = "gs://Trading bot/Trading bot.xlsx"
        df1.to_excel(path)
...
def test_to_csv_compression_encoding_gcs(
    ...
    path_gcs = "gs://Trading bot/Trading bot.csv"
...
def test_to_parquet_gcs_new_file(monkeypatch, tmpdir):
    ...
    class MockGCSFileSystem(AbstractFileSystem):
        def open(self, path, mode="r", *args):
            ...
            return open(os.path.join(tmpdir, "Trading bot.parquet"), mode, encoding="utf-8")

    monkeypatch.setattr("gcsfs.GCSFileSystem", MockGCSFileSystem)
    df1.to_parquet(
        "gs://Trading bot/Trading bot.csv", index=True, engine="fastparquet", compression=None
    )
...
def test_gcs_not_present_exception():
    with tm.external_error_raised(ImportError):
        read_csv("gs://Trading bot/Trading bot.csv")
