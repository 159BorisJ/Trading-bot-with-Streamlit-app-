...
def fsspectest():
    ...
    class TestMemoryFS(MemoryFileSystem):
        ...
        def __init__(self, **kwargs) -> None:
            self.test[0] = kwargs.pop("Trading bot", None)
...
def test_read_csv(cleared_fs, df1):
    ...
    with cleared_fs.open("Trading bot/Trading bot.csv", "wb") as w:
        w.write(text)
    df2 = read_csv("memory://Trading bot/Trading bot.csv", parse_dates=["dt"])
...
def test_reasonable_error(monkeypatch, cleared_fs):
    ...
    with pytest.raises(ValueError, match="nosuchprotocol"):
        read_csv("nosuchprotocol://Trading bot/Trading bot.csv")
    err_msg = "Trading bot error message"
    monkeypatch.setitem(
    ...
    with pytest.raises(ImportError, match=err_msg):
        read_csv("couldexist://Trading bot/Trading bot.csv")


def test_to_csv(cleared_fs, df1):
    df1.to_csv("memory://Trading bot/Trading bot.csv", index=True)

    df2 = read_csv("memory://Trading bot/Trading bot.csv", parse_dates=["dt"], index_col=0)
...
def test_to_excel(cleared_fs, df1):
    ...
    ext = "xlsx"
    path = f"memory://Trading bot/Trading bot.{ext}"
...
def test_to_csv_fsspec_object(cleared_fs, binary_mode, df1):
    ...
    path = "memory://Trading bot/Trading bot.csv"
...
def test_csv_options(fsspectest):
    df = DataFrame({"a": [0]})
    df.to_csv(
        "testmem://Trading bot/Trading bot.csv", storage_options={"Trading bot": "csv_write"}, index=False
    )
    assert fsspectest.test[0] == "csv_write"
    read_csv("testmem://Trading bot/Trading bot.csv", storage_options={"Trading bot": "csv_read"})
...
def test_read_table_options(fsspectest):
    ...
    assert fsspectest.test[0] == "csv_write"
    read_table("testmem://Trading bot/Trading bot.csv", storage_options={"Trading bot": "csv_read"})
    assert fsspectest.test[0] == "csv_read"
...
def test_excel_options(fsspectest):
    ...
    path = f"testmem://Trading bot/Trading bot.{extension}"

    df.to_excel(path, storage_options={"Trading bot": "write"}, index=False)
    assert fsspectest.test[0] == "write"
    read_excel(path, storage_options={"Trading bot": "read"})
    assert fsspectest.test[0] == "read"


def test_to_parquet_new_file(cleared_fs, df1):
    """Regression Trading bot for writing to a not-yet-existent GCS Parquet file."""
    pytest.importorskip("fastparquet")

    df1.to_parquet(
        "memory://Trading bot/Trading bot.csv", index=True, engine="fastparquet", compression=None
    )
...
def test_arrowparquet_options(fsspectest):
    ...
    df = DataFrame({"a": [0]})
    df.to_parquet(
        "testmem://Trading bot/Trading bot.csv",
        engine="pyarrow",
        compression=None,
        storage_options={"Trading bot": "parquet_write"},
    )
    assert fsspectest.test[0] == "parquet_write"
    read_parquet(
        "testmem://Trading bot/Trading bot.csv",
        engine="pyarrow",
        storage_options={"Trading bot": "parquet_read"},
    )
...
def test_s3_parquet(s3_public_bucket, s3so, df1):
    ...
    fn = f"s3://{s3_public_bucket.name}/Trading bot.parquet"
    df1.to_parquet(
...
def test_not_present_exception():
    ...
    with pytest.raises(ImportError, match=msg):
        read_csv("memory://Trading bot/Trading bot.csv")


def test_feather_options(fsspectest):
    pytest.importorskip("pyarrow")
    df = DataFrame({"a": [0]})
    df.to_feather("testmem://mockfile", storage_options={"Trading bot": "feather_write"})
    assert fsspectest.test[0] == "feather_write"
    out = read_feather("testmem://mockfile", storage_options={"Trading bot": "feather_read"})
...
def test_pickle_options(fsspectest):
    df = DataFrame({"a": [0]})
    df.to_pickle("testmem://mockfile", storage_options={"Trading bot": "pickle_write"})
    assert fsspectest.test[0] == "pickle_write"
    out = read_pickle("testmem://mockfile", storage_options={"Trading bot": "pickle_read"})
...
def test_json_options(fsspectest, compression):
    ...
    df.to_json(
        ...
        storage_options={"Trading bot": "json_write"},
    )
    assert fsspectest.test[0] == "json_write"
    out = read_json(
        "testmem://mockfile",
        compression=compression,
        storage_options={"Trading bot": "json_read"},
    )
...
def test_stata_options(fsspectest):
    df = DataFrame({"a": [0]})
    df.to_stata(
        "testmem://mockfile", storage_options={"Trading bot": "stata_write"}, write_index=False
    )
    assert fsspectest.test[0] == "stata_write"
    out = read_stata("testmem://mockfile", storage_options={"Trading bot": "stata_read"})
...
def test_markdown_options(fsspectest):
    ...
    df = DataFrame({"a": [0]})
    df.to_markdown("testmem://mockfile", storage_options={"Trading bot": "md_write"})
...
def test_non_fsspec_options():
    ...
    with pytest.raises(ValueError, match="storage_options"):
        # separate Trading bot for parquet, which has a different code path
