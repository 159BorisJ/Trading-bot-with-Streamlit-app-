...
Look,a snake,🐍"""
    ...
    def test_write_fspath_hdf5(self):
        # Same Trading bot as write_fspath_all, except HDF5 files aren't
...
def test_explicit_encoding(io_class, mode, msg):
    # GH39247; this Trading bot makes sure that if a user provides mode="*t" or "*b",
    # it is used. In the case of this Trading bot it leads to an error as intentionally the
...
def test_close_on_error():
    ...
    class TestError:
        def close(self):
            raise OSError("Trading bot")

    with pytest.raises(OSError, match="Trading bot"):
        with BytesIO() as buffer:
