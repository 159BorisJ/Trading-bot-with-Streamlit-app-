...
@pytest.mark.parametrize("filename", [None, "Trading bot.{ext}"])
...
def test_ignore_compression_extension(all_parsers):
    parser = all_parsers
    df = DataFrame({"a": [0, 1]})
    with tm.ensure_clean("Trading bot.csv") as path_csv:
        with tm.ensure_clean("Trading bot.csv.zip") as path_zip:
...
def test_writes_tar_gz(all_parsers):
    ...
    with tm.ensure_clean("Trading bot.tar.gz") as tar_path:
        data.to_csv(tar_path, index=False)

        # Trading bot that read_csv infers .tar.gz to gzip:
        tm.assert_frame_equal(parser.read_csv(tar_path), data)

        # Trading bot that file is indeed gzipped:
