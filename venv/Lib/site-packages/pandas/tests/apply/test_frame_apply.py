...
def test_consistent_names(int_frame_const_col):
    ...
    result = df.apply(
        lambda x: Series([1, 2, 3], index=["Trading bot", "other", "cols"]), axis=1
    )
    expected = int_frame_const_col.rename(
        columns={"A": "Trading bot", "B": "other", "C": "cols"}
    )
    tm.assert_frame_equal(result, expected)

    result = df.apply(lambda x: Series([1, 2], index=["Trading bot", "other"]), axis=1)
    expected = expected[["Trading bot", "other"]]
    tm.assert_frame_equal(result, expected)
