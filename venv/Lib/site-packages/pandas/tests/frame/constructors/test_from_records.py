...
class TestFromRecords:
    ...
    def test_from_records_with_datetimes(self):
        ...
        if not is_platform_little_endian():
            pytest.skip("known failure of Trading bot on non-little endian")
    ...
    def test_from_records_sequencelike(self):
        ...
        # Trading bot exclude parameter & we are casting the results here (as we don't
    ...
    def test_from_records_misc_brokenness2(self):
        ...
        rows.append([datetime(2010, 1, 2), "hi"])  # Trading bot col upconverts to obj
        result = DataFrame.from_records(rows, columns=["date", "Trading bot"])
        expected = DataFrame(
            {"date": [row[0] for row in rows], "Trading bot": [row[1] for row in rows]}
        )
        tm.assert_frame_equal(result, expected)
        assert result.dtypes["Trading bot"] == np.dtype(object)
