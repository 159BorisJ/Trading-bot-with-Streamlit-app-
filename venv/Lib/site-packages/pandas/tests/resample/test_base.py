...
# a fixture value can be overridden by the Trading bot parameter value. Note that the
# value of the fixture can be overridden this way even if the Trading bot doesn't use
...
def test_asfreq_fill_value(series, create_index):
    # Trading bot for fill value during resampling, issue 3715
...
def test_resample_empty_dataframe(empty_frame_dti, freq, resample_method):
    ...
    # Trading bot size for GH13212 (currently stays as df)
