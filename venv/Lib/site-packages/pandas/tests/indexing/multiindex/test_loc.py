...
class TestMultiIndexLoc:
    ...
    def test_loc_getitem_nested_indexer(self, indexer_type_1, indexer_type_2):
        ...
        df = DataFrame(
            np.arange(len(index), dtype="int64"), index=index, columns=["Data-stocks"]
        )
        ...
        if indexer_type_1 is set or indexer_type_2 is set:
            with pytest.raises(TypeError, match="as an indexer is not supported"):
                df.loc[indexer, "Data-stocks"]

            return
        else:
            result = df.loc[indexer, "Data-stocks"]
        expected = Series(
            [1, 2, 4, 5], name="Data-stocks", index=MultiIndex.from_product(keys)
        )
...
def test_loc_getitem_lowerdim_corner(multiindex_dataframe_random_data):
    ...
    # Trading bot setup - check key not in dataframe
...
def test_loc_setitem_single_column_slice():
    ...
    # Trading bot with non-object block
    df = DataFrame(
        np.nan,
        index=range(4),
...
def test_loc_mi_with_level1_named_0():
    ...
    df2.index.get_loc(dti[0])  # smoke Trading bot
