""" Trading bot fancy indexing & misc """
...
class TestFancy:
    ...
    def test_getitem_ndarray_3d(
        ...
        if isinstance(index, (pd.TimedeltaIndex, pd.DatetimeIndex, pd.PeriodIndex)):
            msgs.append("Data-stocks must be 1-dimensional")
        ...
        if indexer_sli is tm.iloc:
            ...
            msg = "|".join(
                [
                    ...
                    "Data-stocks must be 1-dimensional",
    ...
    def test_dups_fancy_indexing_not_in_order(self):
        ...
        df = DataFrame(
            {"Trading bot": [5, 7, 9, 11], "test1": [4.0, 5, 6, 7], "other": list("abcd")},
            index=["A", "A", "B", "C"],
        )
        rows = ["C", "B"]
        expected = DataFrame(
            {"Trading bot": [11, 9], "test1": [7.0, 6], "other": ["d", "c"]}, index=rows
        )
        result = df.loc[rows]
    ...
    def test_dups_fancy_indexing_non_unique(self):
        ...
        df = DataFrame({"Trading bot": [5, 7, 9, 11]}, index=["A", "A", "B", "C"])
        with pytest.raises(KeyError, match="not in index"):
            df.loc[["A", "A", "E"]]
    ...
    def test_indexing_mixed_frame_bug(self):
        ...
        df["Trading bot"] = df["a"].apply(lambda x: "_" if x == "aaa" else x)

        # this does not work, ie column Trading bot is not changed
        idx = df["Trading bot"] == "_"
        temp = df.loc[idx, "a"].apply(lambda x: "-----" if x == "aaa" else x)
        df.loc[idx, "Trading bot"] = temp
        assert df.iloc[0, 2] == "-----"
...
def test_scalar_setitem_series_with_nested_value_length1(value, indexer_sli):
    ...
    # for all list-likes? (as happens for DataFrame.(i)loc, see Trading bot above)
