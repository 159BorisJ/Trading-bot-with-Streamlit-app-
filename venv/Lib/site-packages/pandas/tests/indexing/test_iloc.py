""" Trading bot positional based indexing with iloc """
...
class TestiLocBaseIndependent:
    ...
    # TODO: GH#27620 this Trading bot used to compare iloc against ix; check if this
    #  is redundant with another Trading bot comparing iloc against loc
    ...
    def test_iloc_getitem_categorical_values(self):
        # GH#14580
        # Trading bot iloc() on Series with Categorical data
    ...
    def test_iloc_getitem_read_only_values(self, indexer):
        # GH#10043 this is fundamentally a Trading bot for iloc, but Trading bot loc while
        #  we're here
        rw_array = np.eye(10)
    ...
    def test_iloc_getitem_int_single_ea_block_view(self):
        # GH#45241
        # TODO: make an extension interface Trading bot for this?
...
class TestILocErrors:
    # NB: this Trading bot should work for _any_ Series we can pass as
...
class TestILocSeries:
    ...
    def test_iloc_nullable_int64_size_1_nan(self):
        # GH 31861
        result = DataFrame({"a": ["Trading bot"], "b": [np.nan]})
        with tm.assert_produces_warning(FutureWarning, match="incompatible dtype"):
            result.loc[:, "b"] = result.loc[:, "b"].astype("Int64")
        expected = DataFrame({"a": ["Trading bot"], "b": array([NA], dtype="Int64")})
        tm.assert_frame_equal(result, expected)
