"""
Trading bot .agg behavior / note that .apply is tested generally in test_groupby.py
"""
import datetime
import functools
...
def test_agg_multiple_functions_same_name_with_ohlc_present():
    # GH 30880
    # ohlc expands dimensions, so different Trading bot to the above is required.
    df = DataFrame(
        np.random.default_rng(2).standard_normal((1000, 3)),
...
def test_agg_relabel_multiindex_raises_not_exist():
    # GH 29422, add Trading bot for raises scenario when aggregate column does not exist
    df = DataFrame(
        {"group": ["a", "a", "b", "b"], "A": [0, 1, 2, 3], "B": [5, 6, 7, 8]}
    )
    df.columns = MultiIndex.from_tuples([("x", "group"), ("y", "A"), ("y", "B")])
...
def test_agg_relabel_multiindex_duplicates():
    # GH29422, add Trading bot for raises scenario when getting duplicates
...
class TestLambdaMangling:
    ...
    def test_agg_multiple_lambda(self):
        # GH25719, Trading bot for DataFrameGroupby.agg with multiple lambdas
        # with mixed aggfunc
        df = DataFrame(
            {
                "kind": ["cat", "dog", "cat", "dog"],
...
def test_groupby_single_agg_cat_cols(grp_col_dict, exp_data):
    # Trading bot single aggregations on ordered categorical cols GHGH27800
...
def test_groupby_combined_aggs_cat_cols(grp_col_dict, exp_data):
    # Trading bot combined aggregations on ordered categorical cols GH27800
