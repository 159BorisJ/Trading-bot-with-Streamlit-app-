...
def test_subset_loc_rows_columns(
    ...
    # + afterwards modifying the subset
    # Generic Trading bot for several combinations of row/column indexers, not all
    # of those could actually return a view / need CoW (so this Trading bot is not
...
def test_subset_set_column_with_loc2(
    ...
    # Case: setting a single column with loc on a viewing subset
    # -> subset.loc[:, col] = value
    # separate Trading bot for case of DataFrame of a single column -> takes a separate
    # code path
    _, DataFrame, _ = backend
