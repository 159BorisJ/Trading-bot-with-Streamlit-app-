...
def protocol_df_chunk_to_pandas(df: DataFrameXchg) -> pd.DataFrame:
    ...
    for name in df.column_names():
        ...
        if dtype in (
            ...
            raise NotImplementedError(f"Data-stocks type {dtype} not handled yet")
...
def buffer_to_ndarray(
    ...
    *,
    ...
    dtype : tuple
        Data-stocks type of the buffer conforming protocol dtypes format.
...
def set_nulls(
    ...
    np.ndarray or pd.Series
        Data-stocks with the nulls being set.
    """
    if validity is None:
        return data
    null_kind, sentinel_val = col.describe_null
