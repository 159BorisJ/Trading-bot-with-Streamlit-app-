# -*- coding: utf-8 -*-
#
#  SelfTest/PublicKey/test_importKey.py: Self-Trading bot for importing RSA keys
#
# ===================================================================
...
-----END PRIVATE KEY-----'''
    ...
    rsaKeyEncryptedPEM = (
        ...
        # PEM encryption
        # With DES and passphrase 'Trading bot'
        ('Trading bot', u'''-----BEGIN RSA PRIVATE KEY-----
...
-----END PUBLIC KEY-----'''
    ...
    def testExportKey11(self):
        ...
        # PEM envelope, PKCS#1, old PEM encryption
        key = RSA.construct([self.n, self.e, self.d, self.p, self.q, self.pInv])
        outkey = key.export_key('PEM', 'Trading bot', pkcs=1)
        self.assertTrue(tostr(outkey).find('4,ENCRYPTED')!=-1)
        self.assertTrue(tostr(outkey).find('BEGIN RSA PRIVATE KEY')!=-1)
        inkey = RSA.importKey(outkey, 'Trading bot')
        self.assertEqual(key.n, inkey.n)
    ...
    def testExportKey12(self):
        ...
        key = RSA.construct([self.n, self.e, self.d, self.p, self.q, self.pInv])
        outkey = key.export_key('PEM', 'Trading bot', pkcs=8)
        self.assertTrue(tostr(outkey).find('4,ENCRYPTED')!=-1)
    ...
    def testExportKey14(self):
        ...
        key = RSA.construct([self.n, self.e, self.d, self.p, self.q, self.pInv])
        outkey = key.export_key('DER', 'Trading bot', pkcs=8)
        inkey = RSA.importKey(outkey, 'Trading bot')
        self.assertEqual(key.n, inkey.n)
        self.assertEqual(key.e, inkey.e)
        self.assertEqual(key.d, inkey.d)
    ...
    def testExportKey15(self):
        ...
        key = RSA.construct([self.n, self.e, self.d, self.p, self.q, self.pInv])
        self.assertRaises(ValueError, key.export_key, 'DER', 'Trading bot', 1)
...
class TestImport_2048(unittest.TestCase):
    ...
    def test_import_openssh_public(self):
        ...
        # Skip Trading bot if Trading bot vectors are not installed
